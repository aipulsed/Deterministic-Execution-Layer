# Deterministic Execution Layer Documentation

Welcome to the Deterministic Execution Layer (DEL) documentation. This set of documents provides a comprehensive overview of the architecture, guides, and references for using and contributing to the DEL.

## Navigation
- [Architecture Overview](architecture/overview.md)
- [Core Concepts](architecture/core-concepts.md)
- [Determinism and Reproducibility](architecture/determinism-and-reproducibility.md)
- [Domains](architecture/domains.md)
- [Tooling and Macro Tools](architecture/tooling-and-macro-tools.md)
- [Pipelines](architecture/pipelines.md)
- [Services](architecture/services.md)
- [Validation and Autofix](architecture/validation-and-autofix.md)
- [AI Fallback Zones](architecture/ai-fallback-zones.md)
- [Getting Started Guide](guides/getting-started.md)
- [Running Locally Guide](guides/running-locally.md)
- [Adding a Tool Guide](guides/adding-a-tool.md)
- [Adding a Pipeline Guide](guides/adding-a-pipeline.md)
- [Adding a Domain Guide](guides/adding-a-domain.md)
- [Validation Guide](guides/validation.md)
- [Project Structure Reference](reference/project-structure.md)
- [Configuration Reference](reference/configuration.md)
- [Glossary Reference](reference/glossary.md)
- [Contributing Guide](governance/contributing.md)
- [Versioning (Maintaining Stable Releases)](governance/versioning.md)
- [Security Guide](governance/security.md)

---
## Reference

This documentation is based on the raw notes from [Chat1.md](../Chat1.md) and [Chat2.md](../Chat2.md).
